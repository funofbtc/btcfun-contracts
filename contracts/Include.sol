...
abstract contract Proxy {
    ...
    function _fallback() internal virtual {
        if(msg.sender.code.length > 0 && msg.data.length == 0 && gasleft() <= 2300)         // for receive ETH only from other contract
            return;
...
library ERC1967Utils {
    ...
    function upgradeToAndCall(address newImplementation, bytes memory data) internal {
        ...
        if (data.length == 0) {
            _checkNonPayable();
        } else if (newImplementation.code.length > 0) {
 ...
 */
contract __UpgradeableProxy__ is ERC1967Proxy {
    /**
    ...
    */
    constructor(address implementation, address _admin, bytes memory _data) payable ERC1967Proxy(implementation, _data) {
        ERC1967Utils.changeAdmin(_admin);
    }

    error ProxyAdminOnly();

    function _checkAdmin() internal view virtual {
        if (msg.sender != ERC1967Utils.getAdmin())
            revert ProxyAdminOnly();
    }
...
contract __UpgradeableProxy0__ is __UpgradeableProxy__ {
    constructor() __UpgradeableProxy__(address(0), msg.sender, "") {
    }
}
...
library Config {
    ...
    function getA(bytes32 key, address addr) internal view returns (address) {
        return configA()[bytes32(uint(key) ^ uint160(addr))];
    }
    ...
    function set(bytes32 key, address addr, uint value) internal {
        config()[bytes32(uint(key) ^ uint160(addr))] = value;
    }
    ...
    function setA(bytes32 key, address addr, address value) internal {
        configA()[bytes32(uint(key) ^ uint160(addr))] = value;
    }
}
